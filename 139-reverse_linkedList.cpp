...
struct node* listReverseR(struct node* head){
    //Base condition - when list is empty or when only a single node
    if(head == NULL || head->next == NULL){
        return head;
    }

    //Space Complexity ~ O(N), bcz while the recursive calls stack been created
    struct node* newHead = listReverseR(head->next);
    head->next->next = head;
    head->next = NULL;
    return newHead;
}
